...
class Attendant {
	...
	*/
	 ...
	 * @return bool
	 */
	public function has_vip_connection_owner() {
		...
		return $current_owner->user_login === $details['login'];
	}
	...
	private function get_expected_owner_details() {
		...
		// NOTE: This filter must be applied very early. Go before plugins_loaded priority 25 to be sure.
		if ( null !== $legacy_owner_details && ! apply_filters( 'vip_jetpack_connection_pilot_use_org_owner', false ) ) {
	...
	private function get_org_owner_details() {
		...
		// TODO: Update email address pattern to be something more like vip-jetpack-owner-{$org_id}@wpvip.com
		return [
			'email'        => "wpcomvip+customer{$org_id}@automattic.com",
	...
	*/
	...
	public function modify_user_capabilties( $caps, $requested_cap, $user_id, $args ) {
		...
		if ( $is_vip_env && in_array( $requested_cap, $caps_to_restrict, true ) ) {
			...
			$org_owner    = $this->get_org_owner_details();
			if ( false !== $user && ! in_array( $user->user_login, [ $legacy_owner['login'], $org_owner['login'] ], true ) ) {
				return [ 'do_not_allow' ];
			}
		}
	...
	public function remove_my_jetpack_page() {
		...
		if ( $this->has_vip_connection_owner() ) {
